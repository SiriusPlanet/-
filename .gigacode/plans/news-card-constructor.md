...
4. **Протестировать** — рендер одной карточки

### 📌 Этап 2: Конструктор 3 (Сборка Страницы)
5. **Создать JS-модуль** `news-page-constructor.js`
6. **Обновить index.html** — подключить новые скрипты (уже есть `ticker-container`)
7. **Протестировать** — рендер всей страницы с карточками и бегущей строкой

### 📌 Этап 3: Интеграция с формой (Конструктор 1)
8. **Обновить JSON-структуру** — добавить `discount` и `category`
9. **Обновить форму** — добавить поля для скидки и категории
10. **Протестировать** — полный цикл: форма → файлы → карточка → страница
...
**Версия:** 4.0 (Три Конструктора - Конструктор-Конструктор-Конструктор)